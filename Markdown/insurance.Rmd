...
```{r SMOTE}
set.seed(1)
SMOTE <- ubBalance(train[,-c(1,11)], train[,11],type="ubSMOTE", positive = 1, 
...
```{r ROC Alberi}
par(pty = "s")
plot.roc(as.numeric(test$Response), as.numeric(prediction_unbalanced), col="red", percent=TRUE)
...
legend("bottomright", legend = c("Unbalanced", "Random Undersampling", "SMOTE"), lty=1, col=c("red", "blue", "green"), cex = 2)
```
...
```{r ROC Naive-Bayes}
par(pty = "s")
plot.roc(as.numeric(test$Response), as.numeric(b_prediction_unbalanced), col="red", percent=TRUE)
